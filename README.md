...
3. ادفع الفرع الحالي (أو `main`) لتفعيل سير العمل والتأكد من أن GitHub Pages لديها مصدر للنشر:
...
### إعادة بناء الفرع الحالي ونشره على GitHub

إذا احتجت إلى إعادة النشر بعد أي تعديل على الفرع النشط (مثل `work`):

1. تأكد من وجود مصدر بعيد باسم `origin` يشير إلى مستودع GitHub الصحيح:

   ```bash
   git remote -v
   ```

   أضف المصدر إذا لم يكن موجودًا:

   ```bash
   git remote add origin git@github.com:<اسم_المستخدم>/<اسم-المستودع>.git
   ```

2. ثبّت الاعتماديات وشغّل البناء للتأكد من سلامة الحزمة:
...
3. انشر الإصدار الحالي يدويًا (اختياري إذا كنت تستخدم GitHub Actions):

   ```bash
   npm run deploy
   ```

4. ادفع الفرع الحالي إلى GitHub لتفعيل GitHub Actions أو تحديث الفرع `gh-pages`:

   ```bash
   git push origin HEAD
   ```

بعدها ستجد التحديثات متاحة على GitHub Pages سواء عبر السير الآلي أو عبر الفرع `gh-pages` الذي أنشأه سكربت النشر اليدوي.
